...
int main(int argc, char **argv) {
    ...
    node_set_label(&leaf1, "leaf 1");
    node_set_actions(&leaf1, &leaf_success);

    Node_t leaf2;
    node_create(&leaf2, LEAF);
    node_set_label(&leaf2, "leaf 2");
    node_set_actions(&leaf2, &leaf_failure);

    Node_t entry;
    node_create(&entry, ENTRY);
    node_set_label(&entry, "entry");

    Node_t inverter;
    node_create(&inverter, INVERTER);
    node_set_label(&inverter, "inverter");

    Node_t sequence;
    node_create(&sequence, FALLBACK);
    node_set_label(&sequence, "sequence");

    node_add_child(&entry, &sequence);
    node_add_child(&sequence, &inverter);
    node_add_child(&inverter, &leaf1);
    node_add_child(&sequence, &leaf2);
